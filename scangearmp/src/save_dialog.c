...
static CNMSInt32	prev_format = CNMS_SAVE_FORMAT_PNG;	/* default */

CNMSInt32 SaveDialog_Open( CNMSVoid )
{
	GtkWidget	*combobox_save_main;
	CNMSInt32	ret = CNMS_ERR, i, j;
	CNMSInt8	extbuf[32];
	...
	for( i = 0 ; i < CNMS_SAVE_FORMAT_MAX ; i ++ ){
...
CNMSInt32 SaveDialog_Show( CNMSVoid )
{
	...
	if ( 0 <= prev_format && prev_format < CNMS_SAVE_FORMAT_MAX ) {
		if( W1_GetSelectSourceADF() == CNMS_TRUE ){
			prev_format = 0;
		}
		gtk_combo_box_set_active( GTK_COMBO_BOX( widget ), prev_format );
	}
	else {
	...
	W1_ModalDialogShowAction( lpSave->save_main_dialog );
...
CNMSVoid SaveDialog_Hide( CNMSVoid )
{
	...
	else if( ( prev_format = gtk_combo_box_get_active( GTK_COMBO_BOX( widget ) ) ) < 0 ){
		DBGMSG( "[SaveDialog_Hide]Can't get ext str.\n" );
		goto	EXIT;
	}
	W1_ModalDialogHideAction( lpSave->save_main_dialog );
...
CNMSInt32 SaveDialog_Exec( CNMSVoid )
{
	...
	GtkWidget	*combobox_save_main;
	CNMSInt32	ret = CNMS_ERR, i, ldata, ExtAddFlag;
	...
	if( W1_GetSelectSourceADF() == CNMS_FALSE ){
		if( ( lpSave->file_format = gtk_combo_box_get_active( GTK_COMBO_BOX( combobox_save_main ) ) ) < 0 ){
