...
SANE_Status cmt_libusb_bulk_write( int index, unsigned char *buffer, unsigned long *size )
{
	...
	if ( ret_bytes < 0 ) { /* error happend. */
		...
		*size = 0;
		DBGMSG( "cmt_libusb_bulk_read() : IO error.\n" );
...
void cmt_network_init( int network )
{
	...
	if( CNNL_SearchPrintersEx( hmdl, nic, CACHE_PATH, max, &found, CNNET_SEARCH_CACHE_ACTIVEONLY, 1, timeout_msec ) == CNNL_RET_SUCCESS ){
		for (j=0; j<found; j++){
