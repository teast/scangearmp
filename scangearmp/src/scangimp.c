...
static void query( void )
{
	...
	nreturn_vals = 0;
	/* get mem */
	if( ( lpBuf = CnmsGetMem( 512 ) ) == CNMSNULL ){
		DBGMSG( "[query]Can't get work memory.\n" );
		goto	EXIT;
	}
	/* copy string */
	/* name */
	CnmsStrCopy( STR_ABOUT_SG, lpBuf, 512 );
	CnmsStrCat( STR_ABOUT_VER, lpBuf, 512 );
	CnmsStrCat( STR_ABOUT_LINUX, lpBuf, 512 );
	/* copy right */
	CnmsStrCopy( STR_ABOUT_CR, lpBuf + 256, 256 );
	CnmsStrCat( STR_ABOUT_AUTHOR, lpBuf + 256, 256 );
	CnmsStrCat( STR_ABOUT_YEAR, lpBuf + 256, 256 );

	gimpapi->p_gimp_install_procedure(
			lpBuf,					/* name(const gchar *) */
			STR_GIMP_BLURB,			/* blurb(const gchar *) */
			STR_GIMP_HELP,			/* help(const gchar *) */
			STR_ABOUT_AUTHOR,		/* author(const gchar *) */
			lpBuf + 256,			/* copy right(const gchar *) */
			STR_ABOUT_YEAR,			/* date(const gchar *) */
			STR_GIMP_MENU_LABEL,	/* menu_label(const gchar *) */
			0,						/* image_types(const gchar *) */
			GIMP_EXTENSION,			/* type(GimpPDBProcType) */
			( sizeof( param ) / sizeof( param[ 0 ] ) ),			/* n_params(gint) */
