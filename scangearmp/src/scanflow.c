		...
		if( ( ret = sane_start( uiInfo->Hand ) ) != SANE_STATUS_GOOD ) {
			...
			if( ret == SANE_STATUS_NO_DOCS ) {
				...
				if ( *pages == 0 ) {
					...
					if( ( ret2 = ChildDialogShow( CHILDDIALOG_ID_NO_PAPER ) ) != COMGTK_GTKMAIN_OK ) {
						/* scan canceled */
...
DBGMSG( "-> CnmsScanFlowShowCalibrationDialog\n");
	...
	if ( pages == 1 ) {
		...
		if ( !CCD ) {
			...
			/* show calibration dialog */
			W1_ModalDialogShowAction( uiInfo->Calibration );
		}
	}
	else {
	...
	if ( pages == 1 ) {
		...
		W1_ModalDialogHideAction( uiInfo->Calibration );
		while( gtk_events_pending() ){
			gtk_main_iteration();
		}
		/* show progress dialog */
