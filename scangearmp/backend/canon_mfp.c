	...
	const char *model )
{
	...
	/*--- SANE_Device ---*/
	cmd_set_usb( &name, canon_mfp_product_ids, &product );
	...
	/* set func-pointer (usb_read, usb_write) */
	if(flag_usb){
	...
	if ( api_ver > 1 ) {
		DBGMSG("<attach> p_canon_set_tblpath() path = %s\n", path );
	...
	/* set product id & USBspeed. */
	DBGMSG("<attach> p_canon_init_scanner() product = %X\n", product);
	if ( api->p_canon_init_scanner( product, speed, &canon_dev_info ) < 0 ) {
	...
	SANE_Handle *h )
{
	...
	if ( api->p_canon_init_scanner( dev->info.product_id, dev->info.speed, NULL ) < 0 ) {
		DBGMSG("ERROR : p_canon_init_scanner() product = %d\n", dev->info.product_id);
		return (SANE_STATUS_INVAL);
	}
